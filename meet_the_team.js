...
function create_elements() {
    ...
    for (i = 0; i < 2; i++) {
        total_add += '<tr>';
        for (j = (2 * i); j < (2 * (i + 1)); j++) {
            total_add += '<td><div><h2>' + all_names[j] + '</h2><img src="images/member' + (j + 1) + '.jpg"><a href="#p' + (j + 1) + '"><i id="p' + (j + 1) + '" class="fa-solid fa-circle-chevron-down"></i></a><p>' + all_descriptions[j] + '</p></div></td>';
        }
        total_add += '</tr>';
    }
